import { useState, useEffect } from 'react'
import { useParams, useNavigate, Link } from 'react-router-dom'
import { getStoreById, getVisitsByStore } from '../services/db'
import Card from '../components/ui/Card'
import Button from '../components/ui/Button'
import { MapPin, Phone, Map, Edit2, ArrowLeft, Calendar, DollarSign, Package, Image } from 'lucide-react'
import { formatDate, formatCurrency, openGoogleMaps, openWhatsApp, openPhone } from '../utils/helpers'
import { STATUS_KUNJUNGAN, STATUS_TAGIHAN } from '../utils/helpers'

export default function StoreDetail() {
  const { id } = useParams()
  const navigate = useNavigate()
  const [store, setStore] = useState(null)
  const [visits, setVisits] = useState([])
  const [loading, setLoading] = useState(true)

  useEffect(() => {
    loadData()
  }, [id])

  async function loadData() {
    try {
      const storeData = await getStoreById(id)
      if (!storeData) {
        navigate('/stores')
        return
      }
      setStore(storeData)

      const visitsData = await getVisitsByStore(id)
      setVisits(visitsData.sort((a, b) => b.tanggal - a.tanggal))
    } catch (err) {
      console.error('Failed to load store:', err)
    } finally {
      setLoading(false)
    }
  }

  if (loading) {
    return (
      <div className="space-y-4">
        <div className="skeleton h-48 rounded-2xl" />
        <div className="skeleton h-32 rounded-2xl" />
        <div className="skeleton h-48 rounded-2xl" />
      </div>
    )
  }

  if (!store) {
    return (
      <div className="text-center py-12">
        <p className="text-gray-500">Toko tidak ditemukan</p>
      </div>
    )
  }

  return (
    <div className="space-y-4">
      {/* Back Button */}
      <button
        onClick={() => navigate('/stores')}
        className="flex items-center gap-2 text-gray-600 dark:text-gray-400 hover:text-primary"
      >
        <ArrowLeft className="w-5 h-5" />
        Kembali
      </button>

      {/* Store Photo */}
      {store.foto ? (
        <div className="relative h-48 rounded-2xl overflow-hidden">
          <img
            src={store.foto}
            alt={store.nama}
            className="w-full h-full object-cover"
          />
        </div>
      ) : (
        <div className="h-48 bg-gray-100 dark:bg-slate-800 rounded-2xl flex items-center justify-center">
          <Image className="w-12 h-12 text-gray-400" />
        </div>
      )}

      {/* Store Info */}
      <Card className="p-4">
        <div className="flex items-start justify-between mb-4">
          <div>
            <h1 className="text-xl font-bold text-gray-900 dark:text-white">
              {store.nama}
            </h1>
            {store.area && (
              <span className="badge-info mt-2">{store.area}</span>
            )}
          </div>
          <Link to={`/stores/${id}/edit`}>
            <Button variant="ghost" size="sm">
              <Edit2 className="w-4 h-4" />
            </Button>
          </Link>
        </div>

        <div className="space-y-3">
          {store.alamat && (
            <div className="flex items-start gap-3">
              <MapPin className="w-5 h-5 text-gray-400 flex-shrink-0 mt-0.5" />
              <div>
                <p className="text-sm text-gray-500 dark:text-gray-400">Alamat</p>
                <p className="text-gray-900 dark:text-white">{store.alamat}</p>
              </div>
            </div>
          )}

          {store.hp && (
            <div className="flex items-start gap-3">
              <Phone className="w-5 h-5 text-gray-400 flex-shrink-0 mt-0.5" />
              <div>
                <p className="text-sm text-gray-500 dark:text-gray-400">Telepon</p>
                <p className="text-gray-900 dark:text-white">{store.hp}</p>
              </div>
            </div>
          )}

          {(store.latitude && store.longitude) && (
            <div className="flex items-start gap-3">
              <Map className="w-5 h-5 text-gray-400 flex-shrink-0 mt-0.5" />
              <div>
                <p className="text-sm text-gray-500 dark:text-gray-400">Lokasi</p>
                <p className="text-gray-900 dark:text-white">
                  {store.latitude}, {store.longitude}
                </p>
              </div>
            </div>
          )}

          {store.catatan && (
            <div className="pt-3 border-t border-gray-100 dark:border-slate-700">
              <p className="text-sm text-gray-500 dark:text-gray-400 mb-1">Catatan</p>
              <p className="text-gray-900 dark:text-white">{store.catatan}</p>
            </div>
          )}
        </div>
      </Card>

      {/* Action Buttons */}
      <div className="grid grid-cols-3 gap-3">
        {store.hp && (
          <>
            <button
              onClick={() => openPhone(store.hp)}
              className="flex flex-col items-center gap-2 p-4 bg-emerald-50 dark:bg-emerald-900/20 rounded-2xl text-emerald-600 dark:text-emerald-400"
            >
              <Phone className="w-6 h-6" />
              <span className="text-xs font-medium">Telepon</span>
            </button>
            <button
              onClick={() => openWhatsApp(store.hp)}
              className="flex flex-col items-center gap-2 p-4 bg-emerald-50 dark:bg-emerald-900/20 rounded-2xl text-emerald-600 dark:text-emerald-400"
            >
              <svg className="w-6 h-6" viewBox="0 0 24 24" fill="currentColor">
                <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z"/>
              </svg>
              <span className="text-xs font-medium">WhatsApp</span>
            </button>
          </>
        )}
        {(store.latitude && store.longitude) && (
          <button
            onClick={() => openGoogleMaps(store.latitude, store.longitude)}
            className="flex flex-col items-center gap-2 p-4 bg-emerald-50 dark:bg-emerald-900/20 rounded-2xl text-emerald-600 dark:text-emerald-400"
          >
            <Map className="w-6 h-6" />
            <span className="text-xs font-medium">Maps</span>
          </button>
        )}
      </div>

      {/* Visit History */}
      <div>
        <div className="flex items-center justify-between mb-3">
          <h2 className="text-lg font-semibold text-gray-900 dark:text-white">
            Riwayat Kunjungan
          </h2>
          <Link to={`/visits/new?storeId=${id}`}>
            <Button size="sm" variant="secondary">
              + Kunjungan
            </Button>
          </Link>
        </div>

        {visits.length === 0 ? (
          <Card className="p-6 text-center">
            <Calendar className="w-10 h-10 text-gray-400 mx-auto mb-2" />
            <p className="text-gray-500 dark:text-gray-400">
              Belum ada kunjungan
            </p>
          </Card>
        ) : (
          <div className="space-y-3">
            {visits.map(visit => (
              <Card key={visit.id} className="p-4">
                <div className="flex items-center justify-between mb-2">
                  <span className="text-sm font-medium text-gray-900 dark:text-white">
                    {formatDate(visit.tanggal)}
                  </span>
                  <span className={`badge ${
                    visit.statusKunjungan === 'dikunjungi' ? 'badge-success' :
                    visit.statusKunjungan === 'tutup' ? 'badge-warning' : 'badge-danger'
                  }`}>
                    {STATUS_KUNJUNGAN[visit.statusKunjungan] || visit.statusKunjungan}
                  </span>
                </div>

                {visit.statusKunjungan === 'dikunjungi' && (
                  <div className="grid grid-cols-2 gap-2 text-sm">
                    <div className="flex items-center gap-2 text-gray-500 dark:text-gray-400">
                      <DollarSign className="w-4 h-4" />
                      {formatCurrency(visit.tagihan)} ({STATUS_TAGIHAN[visit.statusTagihan] || '-'})
                    </div>
                    {visit.retur && visit.retur.length > 0 && (
                      <div className="flex items-center gap-2 text-gray-500 dark:text-gray-400">
                        <Package className="w-4 h-4" />
                        {visit.retur.length} retur
                      </div>
                    )}
                  </div>
                )}

                {visit.catatan && (
                  <p className="text-sm text-gray-500 dark:text-gray-400 mt-2 line-clamp-2">
                    {visit.catatan}
                  </p>
                )}
              </Card>
            ))}
          </div>
        )}
      </div>
    </div>
  )
}